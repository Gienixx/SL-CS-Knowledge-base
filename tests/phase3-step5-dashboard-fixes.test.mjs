...
test('dashboard resolves Zendesk agent IDs through the cached directory', async () => {
  const [html, compatibilityScript, migration, syncEndpoint] = await Promise.all([
    read('dashboard.html'),
  ...
  assert.match(html, /dashboard-concern-compat\.js\?v=4/)
