...
test('Team Attendance uses the compact card design and paginates five records at a time', async () => {
  ...
  assert.match(page, /styles\/team-attendance\.css\?v=16/)
...
test('Team Attendance uses the required hour stat color mapping', async () => {
  const styles = await read('styles/team-attendance.css')
  assert.match(styles, /html\[data-site-theme="light"\] \.team-attendance-stat:nth-child\(1\) strong\{\s*color:var\(--ta-ink\)/)
  assert.match(styles, /html\[data-site-theme="light"\] \.team-attendance-stat:nth-child\(1\) span\{\s*color:var\(--ta-muted\)/)
  assert.match(styles, /html\[data-site-theme="light"\] \.team-attendance-stat:nth-child\(2\) strong,[\s\S]*color:var\(--ta-amber\)/)
  ...
  assert.match(styles, /\.team-attendance-stat:nth-child\(1\) strong,[\s\S]*\.team-attendance-stat:nth-child\(2\) span\{\s*color:#fff/)
})
...
test('Team Attendance does not flag fully classified long overtime records', async () => {
  ...
  assert.match(page, /scripts\/team-attendance\.js\?v=27/)
...
test('Team Attendance provides authorized audited approval and locking actions', async () => {
  ...
  assert.match(verification, /payroll_approval_marker_exists/)
  assert.match(verification, /review_status = 'approved'[\s\S]*payroll_approved_at is null/)
  assert.match(verification, /review_status = 'locked'[\s\S]*payroll_approved_at is not null/)
  assert.match(verification, /approval_log\.entity_id = public\.attendance\.id/)
  assert.match(verification, /review_status not in \('approved', 'locked'\)[\s\S]*payroll_approved_at is not null/)
...
test('Team Attendance displays correction modal and submits through correction RPC', async () => {
  ...
  assert.match(page, /team-attendance\.css\?v=16/)
  assert.match(page, /team-attendance\.js\?v=27/)
