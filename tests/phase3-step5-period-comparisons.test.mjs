...
test('Step 5 migration exposes an authenticated comparison RPC', () => {
  const sql = read('supabase/migrations/20260702_phase3_step5_period_comparisons.sql')
...
test('comparison RPC reuses the Step 4 filtered contract for both periods', () => {
  ...
  assert.equal(sql.match(/public\.get_dashboard_filtered_data\(/g)?.length, 2)
...
test('comparison browser module remains available for later detail comparisons', () => {
...
test('overview defers Zendesk comparison work until a detailed report is opened', () => {
  const dashboard = read('dashboard.html')
  const reportDetail = read('scripts/report-details.js')

  assert.equal(dashboard.includes('dashboard-period-comparisons.js'), false)
  assert.equal(dashboard.includes('dashboard-global-filters.js'), false)
  assert.equal(reportDetail.includes('get_dashboard_filtered_data'), true)
  assert.equal(reportDetail.includes('zendesk'), true)
  assert.equal(reportDetail.includes('google_sheet'), true)
})

test('verification checks function existence and access boundaries', () => {
  const sql = read('supabase/verification/phase3_step5_period_comparisons_check.sql')
