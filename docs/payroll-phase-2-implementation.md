...
| 14 | In progress | July 1–15 is calculated with no blocking exceptions and the rollback-only prepaid scenarios passed again on July 30. The `test test` profile is now payroll-ineligible without losing other test access, and Almar's July 27–31 source schedules are recorded. Restore the approved manual workbook, resolve July 16–31 attendance/open-clock blockers and Almar's two-hour July 29/30 overlap, then complete the employee-by-employee comparison |
