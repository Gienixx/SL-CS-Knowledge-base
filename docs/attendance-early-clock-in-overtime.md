...
- Agents with a released shift cannot bypass the timing rule by submitting an unscheduled clock-in request.
...
- Unscheduled attendance remains available only when the agent has no released shift for that work date.

## Deployment

Apply both migrations after the existing workforce migrations through `2026070802_workforce_timezone_new_york.sql`, in this order:
...
The released-schedule blocker query must return zero rows. All function-definition checks must return `true`, and both audit entries must be present.
...
6. Clock in after the scheduled start. Late minutes must be recorded and pre-shift overtime must remain zero.
7. Attempt clock-in against a draft, cancelled, completed, or rest-day schedule. The request must be rejected.
8. Complete an overnight shift. Clock-out must locate the open attendance record and calculate adjustments from the linked schedule.
9. With no released shift assigned for the work date, confirm that unscheduled attendance remains available.
...
The repository test `tests/attendance-early-clock-in-overtime.test.mjs` verifies the page integration, browser guard, server-side 15-minute gate, combined overtime calculation, and unscheduled-request bypass prevention.
