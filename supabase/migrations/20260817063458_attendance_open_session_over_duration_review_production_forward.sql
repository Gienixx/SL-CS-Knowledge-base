...
-- Production-forward Attendance review extension. This migration is based on
-- the live production schema and does not depend on any divergent historical
-- migration being present locally or marked applied remotely.
...
alter function public.workforce_flag_current_open_attendance_over_duration() owner to postgres;
...
-- Manager-side sweep covers agents who remain offline after crossing the
-- threshold. It accepts no attendance id, so callers cannot target arbitrary
-- records; the existing Team Attendance permission is the authorization
-- boundary. Team Attendance invokes this once before its read-only listing.
...
alter function public.workforce_flag_open_attendance_over_duration() owner to postgres;

revoke all on function public.workforce_flag_open_attendance_over_duration() from public, anon;
grant execute on function public.workforce_flag_open_attendance_over_duration() to authenticated, service_role;

-- Updating the supplemental reason still passes through production's normal
-- version, updated_at, and audit triggers. It must not make an imported
-- payroll snapshot stale when no attendance business field changed.
...
-- Production currently exposes this exact argument signature with a 38-field
-- TABLE result. The final manager_review_reason field is an intentional
-- backward-compatible JSON result extension for the new Team Attendance UI.
-- Keep this listing read-only and STABLE; persistence is performed only by
-- workforce_flag_current_open_attendance_over_duration().
...
alter function public.workforce_list_team_attendance(date, date) owner to postgres;
...
comment on function public.workforce_list_team_attendance(date, date) is
  'Returns the same date-range attendance rows to administrators and active agents; sensitive correction and payroll fields remain administrator-only. Includes the durable open-session-over-20-hours manager-review reason.';
