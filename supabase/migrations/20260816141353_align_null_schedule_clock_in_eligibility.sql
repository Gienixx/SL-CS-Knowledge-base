-- Scope NULL-schedule clock-in eligibility to the active local work date and
-- an active overnight schedule from the prior date. Tomorrow's schedule stays
-- selectable, but must not block today's Unscheduled Work sentinel merely
-- because it is present in the schedule-loading window.
