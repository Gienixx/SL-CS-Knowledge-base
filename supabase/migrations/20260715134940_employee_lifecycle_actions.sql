...
alter table public.profiles
  ...
  add column if not exists account_deleted_by uuid;

comment on column public.profiles.account_deleted_at is
  'When set, the Supabase Auth identity was soft-deleted while this workforce profile and its history were retained.';
...
begin
  ...
  if v_action in ('deactivate', 'delete') then
    update public.profiles set supervisor_id = null, updated_at = now()
