  ...
  with source_events as (
    ...
    select
      ...
      case event.action
        ...
        when 'attendance_deleted' then 'Attendance record deleted'
      ...
      case when actor.is_system_admin then 'System Admin'
        when event.actor_user_id is null then 'System'
          ...
          nullif(trim(actor_login.name), ''),
          'An admin'
        ) end as done_by,
      coalesce(target.full_name, target.employee_id, '') as affected_user,
      case when event.action = 'attendance_deleted' then
        coalesce(target.full_name, 'This employee') || '''s attendance record was deleted. Reason: ' || coalesce(nullif(trim(event.reason), ''), 'No reason provided') || '.'
      else coalesce(nullif(trim(event.reason), ''), nullif(event.after_data ->> 'summary', ''),
        initcap(replace(replace(event.action, '_', ' '), '-', ' ')))
      end as summary,
      case
    ...
    left join auth.users actor_auth on actor_auth.id = event.actor_user_id
    left join public.login actor_login on lower(actor_login.email) = lower(actor_auth.email)
