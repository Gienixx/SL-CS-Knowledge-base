...
begin
  ...
  with source_events as (
    select audit.id, audit.created_at, audit.action, audit.entity_type,
      audit.actor_user_id, audit.entity_id, audit.reason,
    ...
    select event.id, event.created_at,
      ...
      coalesce(actor.full_name, actor.employee_id, 'System') as done_by,
      ...
      case when event.action like '%reject%' or event.action like '%fail%' or event.action like '%void%' then 'Needs attention'
  ...
  select normalized.id, normalized.created_at, normalized.category,
    ...
    normalized.summary, normalized.status, normalized.details
  from normalized
