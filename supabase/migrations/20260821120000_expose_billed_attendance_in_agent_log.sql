-- Expose the same immutable and billed attendance timestamps used by Team
-- Attendance to the authenticated employee's read-only Attendance Log.
...
begin
  ...
  prepaid_rows as (
    select
      ...
      null::timestamptz as original_clock_in,
      null::timestamptz as original_clock_out,
      null::timestamptz as billed_clock_in,
      null::timestamptz as billed_clock_out,
...
comment on function public.workforce_list_my_attendance_log(date, date) is
  'Returns the authenticated employee attendance and approved prepaid schedule rows, including immutable original and manager-adjustable billed timestamps.';
