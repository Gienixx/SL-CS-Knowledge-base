-- Harden attendance deletion as an auditable void. Never hard-delete attendance.
...
begin
  ...
  values (v_actor_user_id, 'attendance_deleted', 'attendance', v_attendance.id,
          to_jsonb(v_attendance), jsonb_build_object('target_employee_id', v_attendance.user_id, 'voided', true), v_reason);

  update public.attendance
  set voided_at = statement_timestamp(), voided_by = v_actor_user_id,
      void_reason = v_reason, updated_at = statement_timestamp()
  where id = v_attendance.id and voided_at is null;
...
as $$
begin
  if auth.uid() is null or not public.workforce_is_admin()
     or not public.workforce_has_permission('view_team_attendance') then
    raise exception 'Administrator access is required.';
  end if;
  return query
  select a.id, a.voided_at, a.voided_by, a.void_reason, a.review_status
...
grant execute on function public.workforce_delete_attendance(uuid, text) to authenticated;
revoke all on function public.workforce_verify_attendance_void(uuid) from public, anon, authenticated;
grant execute on function public.workforce_verify_attendance_void(uuid) to authenticated;
