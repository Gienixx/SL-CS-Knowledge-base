...
returns table (
  ...
  original_clock_out timestamptz, regular_minutes integer,
...
comment on function public.workforce_list_team_attendance(date, date) is
  'Returns permission-scoped history with immutable original timestamps for administrators and only today''s open sessions for regular agents.';
