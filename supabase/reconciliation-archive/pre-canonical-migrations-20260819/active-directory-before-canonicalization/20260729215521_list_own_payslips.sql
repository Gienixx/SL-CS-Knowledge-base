...
begin
  ...
  select coalesce(
    jsonb_agg(
      jsonb_build_object(
        ...
        'payslip_number',
          public.payroll_get_payslip_number(record.id),
        ...
        'prepaid_summary', jsonb_build_object(
          ...
          'added_minutes',
            coalesce(balance.added_minutes, record.prepaid_minutes),
          'applied_minutes',
            coalesce(
              balance.applied_minutes,
              record.applied_prepaid_minutes
            ),
          'closing_minutes',
            coalesce(
              balance.closing_minutes,
              greatest(
  ...
  left join lateral (
    select
      (entry.value ->> 'opening_minutes')::integer
        as opening_minutes,
      (entry.value ->> 'added_minutes')::integer
        as added_minutes,
      (entry.value ->> 'applied_minutes')::integer
        as applied_minutes,
      (entry.value ->> 'closing_minutes')::integer
        as closing_minutes
    from jsonb_array_elements(
      coalesce(
        period.finalization_evidence -> 'prepaid_minute_balances',
        '[]'::jsonb
      )
    ) as entry(value)
