-- Recalculating a closed historical row must not fail merely because the
-- employee has a different, legitimate current session open. The one-open-
-- session guard applies only when the row being recalculated is also open.
...
insert into public.workforce_audit_logs (
  ...
  auth.uid(),
  'closed_attendance_recalculation_enabled',
  'attendance',
  jsonb_build_object(
    'closed_rows_allow_separate_open_session', true,
    'open_row_single_session_guard_retained', true
  ),
  'Allowed historical corrections while preserving the one-open-session rule'
);
