...
begin
  ...
  loop
    -- Paid leave is an independent fixed entitlement.  Prepaid balances are
    -- intentionally not consulted and cannot reduce this 480-minute source.
    v_minutes := 480;
    ...
    values (new.id, 'earning', 'paid_leave_earnings', 'Approved paid leave — worked hours are additional', v_minutes::numeric / 60, v_rate.hourly_rate, v_amount, v_rate.id, v_leave.shift_date, v_leave.id, new.calculation_version,
      jsonb_build_object('source', 'approved_paid_leave', 'leave_type', v_leave.leave_type, 'guaranteed_minutes', 480, 'prepaid_minutes_deducted', 0, 'prepaid_independent', true, 'premium_pay', false), new.calculated_by);
