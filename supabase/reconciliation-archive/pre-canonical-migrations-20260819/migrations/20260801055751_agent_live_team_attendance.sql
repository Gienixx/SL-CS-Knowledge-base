...
begin
  ...
  v_today := (
    now() at time zone coalesce(
      nullif(v_current_profile.timezone, ''),
      'America/New_York'
    )
  )::date;
  ...
  select
    ...
    case
      when not v_is_admin or attendance_row.corrected_by is null then null
...
comment on function public.workforce_list_team_attendance(date, date) is
  'Returns permission-scoped history for administrators and only today''s open sessions for regular agents, including prior-night clock-ins assigned to today''s shift.';
