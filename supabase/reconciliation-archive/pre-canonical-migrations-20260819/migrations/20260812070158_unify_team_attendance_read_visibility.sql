-- Keep Team Attendance row visibility identical for administrators and agents.
-- Agent-facing sensitive fields remain redacted by the canonical RPC.
...
begin
  ...
  if not found then
    raise exception using errcode = '42501', message = 'An active workforce profile is required.';
  end if;
...
comment on function public.workforce_list_team_attendance(date, date) is
  'Returns the same date-range attendance rows to administrators and active agents; sensitive correction and payroll fields remain administrator-only.';
