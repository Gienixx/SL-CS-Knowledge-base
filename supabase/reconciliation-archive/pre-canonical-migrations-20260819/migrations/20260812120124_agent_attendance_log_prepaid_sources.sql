-- Employee-scoped Attendance Log source rows for approved prepaid schedules
-- and actual FIFO fulfillment. This is read-only and does not create records.

begin;

create or replace function public.workforce_list_my_attendance_log(
  p_start_date date,
  p_end_date date
)
returns table (
...
begin
  ...
  with actual_rows as (
    select
      ...
      coalesce(allocations.fulfilled_minutes, 0)::integer
        as fulfilled_prepaid_minutes,
      greatest(
        case
...
comment on function public.workforce_list_my_attendance_log(date, date) is
  'Returns only the authenticated employee''s attendance and approved prepaid schedule rows, with FIFO allocation minutes and no payroll amounts.';
