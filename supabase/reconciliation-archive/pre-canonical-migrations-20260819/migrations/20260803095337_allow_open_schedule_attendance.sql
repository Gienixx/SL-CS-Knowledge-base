-- Allow attendance to be linked to an open schedule. Open schedules have no
-- fixed interval, so worked duration is retained while schedule-derived
-- regular, overtime, lateness, and undertime values remain unclassified.
...
comment on function public.workforce_recalculate_attendance(uuid) is
  'Recalculates trusted attendance totals, including unclassified worked duration for open schedules without fixed shift times.';
...
insert into public.workforce_audit_logs (
  ...
  auth.uid(),
  'open_schedule_attendance_enabled',
  'attendance',
  jsonb_build_object(
    'open_schedule_attendance_allowed', true,
    'fixed_interval_calculations_unclassified', true
  ),
  'Allowed attendance records on open schedules without inventing fixed shift calculations'
);
